<script lang="ts">
	...
	export function fireSwalTeacherInfo() {
		Swal.fire({
			title: 'Error!',
			text: 'ครูสายรุ้งใช้เลขที่ 100 ครูเต๋า 101 ครูปุ๊ก 102',
			icon: 'info',
			cancelButtonText: 'OK',
			cancelButtonColor: '#d33',
			showCancelButton: false,
	...
	<div class="py-8 lg:py-16 px-4 mx-auto max-w-screen-md">
		...
		<form class="space-y-8" method="POST" use:enhance>
			<div>
				<label
					for="classNumber"
					class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">เลขที่ (ครูสายรุ้งใช้เลขที่ 34 ครูเต๋า 35 ครูปุ๊ก 36)</label
				>
				<input
					id="classNumber"
					name="classNumber"
			...
			<div class="sm:col-span-2">
				<label
					for="requestText"
					class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-400">อยากได้อะไร</label
				>
				<textarea
					id="requestText"
					name="requestText"
			...
			<button
				type="submit"
				class="py-3 px-5 text-sm font-medium dark:text-white text-center rounded-lg bg-primary-700 sm:w-fit hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800"
